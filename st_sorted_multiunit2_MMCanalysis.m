function [MMCsorted,MMCsortedall,MMCsortedavg] = st_sorted_multiunit2_MMCanalysis(varargin)
clear all; close all; clc;

%{
function to analyze MMC multiunit sorted data
includes group analysis based on st_sorted_MMCanalysis

ST. v1:            01.07.15
Last modified:     08.07.15
Backup:            st_analyze_multiunit2_MMCsorted_copy
...
%         load pp; for jj=pp % pp = find(rate > threshold)
        ...
        MMCsortedavg.Cavgspikeratebaseline      = [MMCsortedavg.Cavgspikeratebaseline MMCsorted.Cavgspikeratebaseline];
%         MMCsortedavg.RCavgspikeratebaseline     = [MMCsortedavg.RCavgspikeratebaseline nanmean(MMCsorted.RCavgspikeratebaseline{jj})];
%         MMCsortedavg.RefRCavgspikeratebaseline  = [MMCsortedavg.RefRCavgspikeratebaseline nanmean(MMCsorted.RefRCavgspikeratebaseline{jj})];
%         
...
%         MMCsortedavg.RefRCclickavgspikerateperclicklast5 = [MMCsortedavg.RefRCclickavgspikerateperclicklast5 nanmean(MMCsortedall.RefRCclickavgspikerateperclicklast5{jj})];
    ...
    if(savegroupdataflag)
        save([MMCsorted.savepath 'Results/GroupResults' filesep 'GroupResults.mat'],'MMCsortedall','MMCsortedavg');
    end
